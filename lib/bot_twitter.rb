...
def like_follow_live
  client = login_stream_twitter
  client2 = login_twitter
  topic = "#bonjour_monde"
  client.filter(track: topic) do |tweet|
    if tweet.is_a?(Twitter::Tweet)
      puts tweet.text
      client2.favorite(tweet)
      puts "Le Tweet a été ajouté en fav"
      client2.follow(tweet.user)
      puts "L'utiliser qui a tweeté est maintenant follow"
    end
  end
end
